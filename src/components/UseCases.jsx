          ...
          {useCases.map((useCase, index) => {
            return (
              <article key={useCase.title} className="border-t border-black/[0.12] pt-5">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-forest">0{index + 1}</p>
                <h3 className="mt-4 text-2xl font-semibold tracking-tight text-ink">{useCase.title}</h3>
                <div className="mt-6 space-y-4">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.16em] text-clay">Pain point</p>
                    <p className="mt-1 leading-7 text-[#4e5752]">{useCase.problem}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.16em] text-steel">GridReady output</p>
                    <p className="mt-1 leading-7 text-[#4e5752]">{useCase.help}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.16em] text-forest">Decision improved</p>
